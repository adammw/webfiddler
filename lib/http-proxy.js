...
var HttpProxy = module.exports = exports = function httpProxy(options) {
  ...
  this.middleware = function httpProxyMiddleware(request, response, next){
    ...
    requestId++;
    self.emit("requestWillBeSent", {requestId: requestId, request: {url: request.url, method: request.method, headers: request.headers}, timestamp: timestamp(), client: request.connection.remoteAddress});
